...
def fun():
    ...
    train_loader, test_loader = data_loader.dataloader('under_sampling', batch_size)

    cnt_progress = len(train_loader) // 30

    model_name = 'ensemble_resnet18'
    ...
    class ResNetFeatureExtractor1(nn.Module):
        def __init__(self):
            super(ResNetFeatureExtractor1, self).__init__()
    ...
    class EnsembleModel(nn.Module):
        def __init__(self, modelA, modelB, modelC):
            ...
            self.modelB = modelB
            self.modelC = modelC
            self.classifier_category = nn.Linear(300, 65)
            self.classifier_domain = nn.Linear(300, 4)
        ...
        def forward(self, x):
            ...
            out_class = self.classifier_category(x)
            out_domain = self.classifier_domain(x)
            return out_class, out_domain

    model1 = ResNetFeatureExtractor().to(device)
    model2 = ResNetFeatureExtractor1().to(device)
    model3 = ResNetFeatureExtractor2().to(device)
